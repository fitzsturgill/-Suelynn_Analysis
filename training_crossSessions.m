...
targetLightOn=cell2mat(TE.TargetLightOn);
for i=1:length(targetLightOn)
    pokeIns{i}=TE.Port2In{i}<targetLightOn(i,2);
end

init_pokeIns=cellfun(@sum, pokeIns);
init_pokeIns(init_pokeIns==0)=NaN;

pokeIns_binned=binnedTrials(init_pokeIns, binSize, 'average');
...
%% plot foreperiod times 
        ...
        set(foreperiodAxes, 'XLim', [0 max(xData)], 'YLim', [0 max(foreperiodBinned)]);
        ...
        % gather scatter graphic object handles as numeric values into a
        % numeric array
        pokeIns_handles = scatter(xData, pokeIns_binned); hold on;


        set(pokeInsaxes, 'XLim', [0 max(xData)], 'YLim', [0 max(pokeIns_binned)]);
        % make legend label axes
        % use hanles to create a labeled legend
...
%% plot trial outcomes (correct and incorrect trials)
        ...
        set(reactionTimeAxes, 'XLim', [0 max(xData)], 'YLim', [0 max(reactionTime_binned)] );
        % make legend label axes
        % use hanles to create a labeled legend
