...
if nargin < 1 || isempty(TE) % initialize from scratch (reload)
    ...
    TE = struct(...
        ...
        TE(1).Port2In = bpAddEventAsTrialEvent(sessions, 'Port2In');
...
%         TE.Port2In(tcounter)=length(session.SessionData.RawEvents.Trial{1,counter}.Events.Port2In);
...
TE.trialOutcome(targetLightNotReached) = 0; % early withdrawal
